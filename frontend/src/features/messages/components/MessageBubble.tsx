    ...
    return (
        <div className={`flex w-full mb-4 ${isMine ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex flex-col max-w-[80%] ${isMine ? 'items-end' : 'items-start'}`}>
                ...
                {!isMine && (
                    <span className="text-xs text-muted-foreground mb-1 ml-1 px-1">
                        {message.sender.name}
                    </span>
                )}
                ...
                <div
                    className={`relative rounded-2xl px-4 py-2 text-sm shadow-sm ${isMine
                        ...
                        : 'bg-card dark:bg-card-900 border border-gray-100 dark:border-gray-800 text-foreground dark:text-gray-100 rounded-tl-sm'
                ...
                <div className={`flex items-center gap-1 mt-1 text-[10px] text-gray-400 px-1`}>
                    ...
                    {isMine && message.status === 'failed' && (
                        <button
                            ...
                            className="flex items-center text-destructive hover:text-red-600 ml-1"
                        >
