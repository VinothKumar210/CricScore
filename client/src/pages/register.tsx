      ...
      <div className="w-full max-w-md bg-gradient-to-b from-blue-50 to-blue-100 dark:from-slate-900 dark:to-slate-800 rounded-[3rem] shadow-2xl overflow-hidden flex flex-col relative border-[8px] border-white dark:border-slate-800">
        ...
        <div className="flex justify-between items-center px-8 pt-6 pb-2 w-full text-slate-600 dark:text-slate-400">
          ...
          <div className="flex gap-1">
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15h-2v-6h2v6zm0-8h-2V7h2v2z" /></svg>
          </div>
        </div>
        ...
        <div className="flex-1 px-8 pt-4 pb-12 flex flex-col items-center">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2 font-display">Join CricScore</h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm">Create account to track your cricket career</p>
          </div>
          ...
          >
            {isGoogleLoading ? (
              <span className="text-primary font-semibold text-sm">Signing up...</span>
            ) : (
              <>
                ...
                <span className="text-primary font-semibold text-sm">Sign up with Google</span>
              </>
            )}
          </button>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="w-full space-y-4">
            <div>
              <label className="block text-slate-800 dark:text-slate-200 text-sm font-semibold mb-2 ml-1">Username</label>
              <input
                {...form.register("username")}
                type="text"
                placeholder="cric_lover_99"
              ...
              {form.formState.errors.username && (
                <p className="text-red-500 text-xs mt-1 ml-1">{form.formState.errors.username.message}</p>
              )}
            </div>
            ...
            <div>
              <label className="block text-slate-800 dark:text-slate-200 text-sm font-semibold mb-2 ml-1">Password</label>
              <input
