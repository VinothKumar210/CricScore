...
// Live match schemas for real-time scoring and spectator notifications
export const LiveMatchStatusSchema = z.enum(["SETUP", "LIVE", "COMPLETED", "CANCELLED"]);

export const insertLiveMatchSchema = z.object({
  creatorId: z.string(),
  matchName: z.string().min(1, "Match name is required"),
  venue: z.string().min(1, "Venue is required"),
  matchDate: z.date(),
  status: LiveMatchStatusSchema.default("SETUP"),
  myTeamName: z.string().min(1, "Team name is required"),
  opponentTeamName: z.string().min(1, "Opponent team name is required"),
  overs: z.number().int().min(1).max(50).default(20),
  spectatorUsernames: z.array(z.string()).default([]), // Usernames of players who should receive notifications
  currentScore: z.object({
    runs: z.number().int().min(0).default(0),
    wickets: z.number().int().min(0).max(10).default(0),
    overs: z.number().min(0).default(0),
    ballsInOver: z.number().int().min(0).max(5).default(0),
  }).optional(),
  currentBatsmen: z.object({
    striker: z.string().optional(),
    nonStriker: z.string().optional(),
  }).optional(),
  currentBowler: z.string().optional(),
  battingTeam: z.enum(["MY_TEAM", "OPPONENT"]).optional(),
});

export const insertLiveMatchBallSchema = z.object({
...
export const insertLiveMatchSpectatorSchema = z.object({
  liveMatchId: z.string(),
  username: z.string(),
  userId: z.string().optional(), // If user has account
  notificationSent: z.boolean().default(false),
});

// Frontend form schema for live match creation
export const liveMatchFormSchema = insertLiveMatchSchema.omit({ creatorId: true }).extend({
  matchDate: z.string(),
});
...
export type InsertLiveMatchBall = z.infer<typeof insertLiveMatchBallSchema>;
export type InsertLiveMatchSpectator = z.infer<typeof insertLiveMatchSpectatorSchema>;
export type LiveMatchForm = z.infer<typeof liveMatchFormSchema>;
