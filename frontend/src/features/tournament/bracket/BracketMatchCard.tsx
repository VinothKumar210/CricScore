...
export const BracketMatchCard: React.FC<BracketMatchCardProps> = React.memo(({
    ...
    const getTeamStyle = (teamId: string | null) => {
        if (!teamId || !result) return 'text-textPrimary font-medium';
        if (teamId === winnerId) return 'text-brand font-black'; // Increased contrast for winner
        return 'text-textSecondary line-through opacity-60';
    };
    ...
    return (
        <div
            className="relative bg-surface outline outline-1 outline-border/60 rounded-xl overflow-hidden
        ...
        >
            {/* Stage label */}
            <div className="flex items-center justify-between px-3 py-1.5 bg-gray-50 border-b border-border/30">
                ...
                {result && !isLive && (
                    <span className="text-[9px] font-bold text-green-600">✓ Completed</span>
                )}
            </div>

            {/* Team A */}
            <div className={`flex items-center justify-between px-3 py-2.5 border-b border-border/20
                            ${match.teamAId === winnerId ? 'bg-brand/5' : ''}`}>
                ...
                {match.dependsOn && !match.teamAId && (
                    <span className="text-[8px] text-textSecondary italic">
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2
                            w-5 h-5 rounded-full bg-surface border border-border/50
                            flex items-center justify-center z-10">
                <span className="text-[7px] font-black text-textSecondary">VS</span>
            </div>

            {/* Team B */}
            <div className={`flex items-center justify-between px-3 py-2.5
                            ${match.teamBId === winnerId ? 'bg-brand/5' : ''}`}>
                <div className="flex items-center gap-2 min-w-0">
